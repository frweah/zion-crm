-- ─────────────────────────────────────────────────────────────
-- 0074 — the nightly alerts use the practice's date, and a test can set it
--
-- generate_notifications() decided "today" with current_date: the database
-- server's date, which is UTC. Everything else in the CRM decides it with
-- public.practice_today(), which is Utah's. The nightly job runs at 13:00 UTC,
-- when the two agree, so the scheduled alerts have been right. But any run
-- between six in the evening and midnight in Utah - a manual run, or the
-- verification script - dated every rule a day ahead: invoices a day older,
-- a W-8BEN expiring tonight called expired, the monthly-forms window and the
-- January 1099 season a day early. It surfaced as verify_notifications
-- failing at 22:40 local.
--
-- The rules move unchanged into generate_notifications_on(p_today), which
-- dates everything from the day it is given (or the practice's today when
-- given nothing). generate_notifications() keeps its name, arguments and
-- grants, and now passes the practice's today - so pg_cron, the app and the
-- generated types need no change. Tests can pin the day.
--
-- Generated from the live pg_get_functiondef output on 2026-09-13, like 0058,
-- with exactly one line changed: the declaration of v_today.
-- ─────────────────────────────────────────────────────────────
...
revoke all on function public.generate_notifications_on(date) from public;
-- Choosing the day is for the job and the tests, not for anybody signed in.
-- generate_notifications() keeps exactly the grants it had. Revoking from
-- public is not enough on its own: Supabase grants anon and authenticated
-- on every new function by default, which the first apply of this migration
-- left in place.
revoke execute on function public.generate_notifications_on(date) from anon, authenticated;
grant execute on function public.generate_notifications_on(date) to service_role;

-- The name everything already calls.
...
comment on function public.generate_notifications_on(date) is 'The nightly alert rules, dated from p_today (the practice''s today if null). generate_notifications() calls this with public.practice_today().';
