-- ─────────────────────────────────────────────────────────────
-- 0076 — an alert that escalates changes its kind, not only its wording
--
-- The nightly reconcile updates an alert that is still current by its
-- dedupe key. It updated the level, text, roles and link - but not the kind.
-- So an alert whose key covers two states kept the first one:
--
--   a W-8BEN that passed its last valid day stayed "w8ben_expiring", with
--   level "bad" and text saying it had expired;
--
--   an authorization whose hours ran out stayed "auth_low" rather than
--   "auth_exhausted".
--
-- Anything that reads the kind - the dashboard, the email digest - was told
-- the milder thing. Found by verify_notifications, which now moves the day
-- past a W-8BEN's expiry and checks the kind.
--
-- The live pg_get_functiondef output of generate_notifications_on from
-- 2026-09-13, with one change: kind = excluded.kind in the reconcile.
